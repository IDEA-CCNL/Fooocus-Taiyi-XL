...
def clip_encode_single(clip, text, verbose=False):
    # 获取clip的属性
    print("clip artributes:", clip.__dict__["tokenizer"].__dict__)
    _clip_tokenizer = clip.__dict__["tokenizer"].__dict__
    # print(_clip_tokenizer["clip_l"].__dict__)
    # print(_clip_tokenizer["clip_g"].__dict__)
    # 把_clip_tokenizer["clip_l"].__dict__转为dict
    clip_l_dict = dict(_clip_tokenizer["clip_l"].__dict__)
    clip_g_dict = dict(_clip_tokenizer["clip_g"].__dict__)
    for k,v in clip_l_dict.items():
        try:
            clip_l_dict[k] = str(v)
        except:
            clip_l_dict[k] = str(v.__dict__)
    for k,v in clip_g_dict.items():
        try:
            clip_g_dict[k] = str(v)
        except:
            clip_g_dict[k] = str(v.__dict__)
    # 保存__dict__为json
    with open("clip_l.json","w") as f:
        json.dump(clip_l_dict,f, ensure_ascii=False)
    with open("clip_g.json","w") as f:
        json.dump(clip_g_dict,f, ensure_ascii=False)
